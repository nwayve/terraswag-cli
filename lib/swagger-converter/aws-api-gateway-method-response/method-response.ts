...
const TEMPLATE = `\
resource "aws_api_gateway_method_response" "{{response.name}}_{{response.statusCode}}" {
...
export class AwsApiGatewayMethodResponse {
    template = compile(TEMPLATE);

    constructor(private config: MethodResponseConfiguration) {}
...
export interface MethodResponseConfiguration {
    name: string;
    serviceName: string;
    statusCode: string;
    resourceName?: string;
